...
namespace caffe {
  ...
  void TripletLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    ...
    caffe_set(summer_vec_.count(), Dtype(1.), summer_vec_.mutable_cpu_data());

    dist_binary_.Reshape(bottom[0]->num(), 1, 1, 1);
    caffe_set(dist_binary_.count(), Dtype(1.), dist_binary_.mutable_cpu_data());
  ...
  void TripletLossLayer<Dtype>::Forward_cpu(
    ...
    loss = loss / Dtype(bottom[0]->num()) / Dtype(2);
