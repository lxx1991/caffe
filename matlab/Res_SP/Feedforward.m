...
file_model = [dir_model, 'model/unary_iter_9000.caffemodel'];
file_def_model = [dir_model, 'test/Res_SP_VOC_test.prototxt'];

results_name = 'Res_SP_VOC';
...
for id_img = 1:num_img
    ...
    tic;
    input_label = zeros(size(im));
    fid = fopen('/DATA3/gSLICr/temp.txt', 'wb');
    fwrite(fid, height_img, 'int');
    fwrite(fid, width_img, 'int');
    fwrite(fid, reshape(permute(im, [2, 1, 3]), height_img * width_img, [])', 'uint8');
    ...
    input_data = {input_img, input_sp, sp_label};
...
%     scores = scores + scores2{1}(end:-1:1, :, :);
    ...
    [~, scores] = max(permute(scores, [2, 1, 3]), [], 3);
    maxlabel = zeros(size(s_sp), 'uint8');
    for i = 0 : length(scores) - 1
        maxlabel(s_sp == i) = scores(i + 1) - 1;
    end;
    
    maxlabel = imresize(maxlabel, [pre_height_img, pre_width_img], 'nearest');
