...
file_model = [dir_model, 'model/unary_local_iter_9000.caffemodel'];
file_def_model = [dir_model, 'test/Res_SP_Local_VOC.prototxt'];

results_name = 'Res_SP_Local_con_VOC';
...
for id_img = 1:num_img
    ...
    input_label = zeros(size(im, 1), size(im, 2));
...
%     system('/DATA3/gSLICr/build/demo /DATA3/gSLICr/temp.txt /DATA3/gSLICr/seg_temp.pgm');
%     input_sp = imread('/DATA3/gSLICr/seg_temp.pgm');
%     [sp_label, input_sp]= sp_graph(double(input_sp), double(input_label));
%     filler1 = gen_local_filler(imresize(input_sp, 1/2, 'nearest'), 5);
%     filler2 = gen_local_filler(imresize(input_sp, 1/4, 'nearest'), 5);
%     s_sp = input_sp;
%     fclose(fid);
%     toc;
