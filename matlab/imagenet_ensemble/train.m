...
%%
dir_model = fullfile('..', '..', 'examples', 'imagenet_ensemble');
file_solver = fullfile(dir_model, 'solver.prototxt');
file_weight = fullfile(dir_model, 'model.caffemodel');
...
while (caffe_solver.iter() < caffe_solver.max_iter())
    ...
    path = h5read(fullfile(hdf5_dir, [num2str(t-1), '.h5']), '/image_filename');
    bbox_pred = h5read(fullfile(hdf5_dir, [num2str(t-1), '.h5']), '/bboxes');
