...
static void solver_get_iter_size(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
    "Usage: caffe_('solver_get_iter_size', hSolver)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  plhs[0] = mxCreateDoubleScalar(solver->iter_size());
}
...
// Usage: caffe_('solver_one_iter_step', hSolver)
static void solver_one_iter_step(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
    "Usage: caffe_('solver_one_iter_step', hSolver)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  solver->OneIterStep();
...
static handler_registry handlers[] = {
  ...
  { "solver_one_iter_step",           solver_one_iter_step          },
