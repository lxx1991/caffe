...
void sp_graph(mxArray *plhs[], double *x, int n, int m, int step, double *label, double *instance)
{
  ...
  for (i=0; i<num; i++)
  {
    int cnt_temp = cnt_class[i * num_class];  
    for (j=1; j<num_class; j++)
    {
      cnt_temp += cnt_class[i * num_class + j];
      if (cnt_class[i * num_class + j] > cnt_class[i * num_class + sp_class[i]])
        sp_class[i] = j;
    }
    if (cnt_class[i * num_class + sp_class[i]] * 2 < cnt_temp)
        sp_class[i] = 255;
  ...
  for (i=0; i<num; i++)
  {
    ...
    if (sp_class[i]!=0 && sp_class[i]!=255)
    {
        for (j=1; j<=t; j++)
            if (sp_class[q[j]]!=255 && sp_class[q[j]] == sp_class[i])
            {
                assert(MAX_EDGE > output_size);
                output[output_size][0] = i;
                output[output_size][1] = q[j];
                output[output_size++][2] = (sp_instance[i] == sp_instance[q[j]]);
            }
        /*for (k=1; k<=t; k++)
            if (sp_instance[i] == sp_instance[q[k]] &&sp_class[q[k]]!=255 && sp_class[q[k]] == sp_class[i])
            {
                assert(MAX_EDGE > output_size);
                output[output_size][0] = i;
                output[output_size][1] = q[k];
                output[output_size++][2] = 1;
            }*/
    }
  }
